...
<title>我的品牌</title>
<link rel="stylesheet" type="text/css" href="<%=basePath%>css/brand/mybrand.css" />
...
function getThirdClass() {
    ...
    $.ajax({
        ...
        data: {"id": id},
        success: function (data) {
            $('#thirdClass').append("<option value='' selected='selected' >" + '请选择' + "</option>");
            for (var i = 0; i < data.length; i++) {
                $('#thirdClass').append("<option value='" + data[i].thirdClass_id + "' >" + data[i].thirdClass_name + "</option>");
            }
        },
        error: function () {
            alert("加载三级业态失败");
...
<div id="top">
	...
	<div id="main">
		<div id="key"><span style="color:#2779B0">商品品牌库</span><span>>我的品牌</span></div>
		<div id="content">
		<form action="<%=path%>/brand/getBrandListByQuery" method="post" id="testForm">
		<!-- 第一行 -->
			<div style="width:100%;height:80px;">
				<div class="div1">一级业态:</div>
				<select class="first"  id="firstClass" onchange="getSecondClass()" style="width: 175px;" name="firstClass_format">
					<option value="">全部</option>
				</select>
				<div class="div2">二级业态:</div>
				<select class="second" id="secondClass" onchange="getThirdClass()" style="width: 175px;" name="secondClass_format">
					<option value="">全部</option>
				</select>
			...
			<div style="width:100%;height:50px;">			
				...
				<select name="consumer_class" class="consumer_class" id="consumer_class" >
					<option value="">全部</option>
			...
			<div style="width:100%;height:50px;">			
				...
				<select name="brand_runtype" class="brand_runtype" id="brand_runtype" >
					<option value="">全部</option>
				</select>
			</div>
			<!-- 第5行 -->
			...
			<div style="width:100%;height:50px;">			
				...
				<input name="key_word" class="key_word"  type="text" placeholder="请输入搜索关键字"/>
			</div>
			</form>
			<!-- 第7行 -->
			<div style="width:100%;height:70px;">			
				<button id="inquire">查询</button>
				<button id="output" type="button" onclick="tableToExcel('item','data')">导出Excel</button>
			...
			<!-- 表格 -->
				...
				<table class="table" border="1" cellspacing="0" cellpadding="5" frame="hsides" id="item">
